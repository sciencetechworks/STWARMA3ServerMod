...
_cache= if (count _this > 6) then {_this select 6} else {false};
	...
	if (_mA==0) then {_mAH = 0.5;_mAN = 0.2;}; //STW
	if (_mA==1) then {_mAH = 0;_mAN = 0;};
	if (_mA==2) then {_mAH = 0.4;_mAN = 0.2;}; //STW
...
if ((_fSize select 0) > 0) then {
	_cargoGrp = createGroup _side;
		...
		_fGroup set [count _fGroup,_cargoGrp];
			null = [_mkr,_fGroup,_counter] execvm "a3_custom\STWEOS\functions\TransportUnload_fnc.sqf";
				}else{
				...
				while {triggeractivated _eosActivated AND !(getmarkercolor _mkr == "colorblack")} do 
						{
							...
							{
									diag_log format ["Under attack %1",_mkr];
									_mkr setmarkercolor hostileColor;
									_mkr setmarkerAlpha _mAH;
									_markerName=  _mkr splitString "@";
									_stwCityName=_markerName select 2;
									_index=STW_CONQUERED_CITIES find _stwCityName;
									if (_index>=0) then
										{
										 STW_CONQUERED_CITIES deleteAt _index;
										 diag_log format["STW LOST CITY=%1",_stwCityName];
										 _stwPersistenceDatabase = ["new", "inidbi"] call OO_PDW;
										 _dbName="STWPERSISTENCE_EOS";
										 ["setDbName", _dbName] call _stwPersistenceDatabase;
									_bool = ["write",["STW_CONQUERED_CITIES", STW_CONQUERED_CITIES]] call _stwPersistenceDatabase;			
										 diag_log format["STW CONQUERED CITY PERSISTENCE %1",_bool];
										};
								if (_debug) then {hint "Zone Lost";};
											...
											if (! (_stwCityName in STW_CONQUERED_CITIES)) then
											{
												STW_CONQUERED_CITIES pushBack _stwCityName;	
												diag_log format["STW CONQUERED CITY=%1",_stwCityName];	
...
if (!(getmarkercolor _mkr == "colorblack")) then {	
	null = [_mkr,[_aGrps,_aSize],[_bGrps,_bSize],[_cGrps,_cSize],[_dGrps,_eGrps,_fGrps,_fSize],_settings,true] execVM "a3_custom\STWEOS\core\eos_core.sqf";
